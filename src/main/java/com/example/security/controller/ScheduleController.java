...
import com.example.security.service.AnniversaryService;
import com.example.security.service.ScheduleService;
import com.example.security.util.SchedUtil;
	...
	public String calendar(@PathVariable(required=false) String arrow, HttpSession session, Model model) {
		...
		if (startDate != 0) {		// 지난 달
			...
			for (int i = 0; i < startDate; i++) {
				...
				SchDay sd = schedService.generateSchDay(sessSuid, prevDay+i, sdate, i, 1);
				week.add(sd);
			}
		}
		for (int i = startDate, k = 1; i < 7; i++, k++) {		// 이번 달
			sdate = String.format("%d%02d%02d", year, month, k);
			SchDay sd = schedService.generateSchDay(sessSuid, k, sdate, i, 0);
			week.add(sd);
		}
		calendar.add(week);
		...
		for (int k = day, i = 0; k <= lastDay.getDayOfMonth(); k++, i++) {
			if (i % 7 == 0)
				week = new ArrayList<>();
			sdate = String.format("%d%02d%02d", year, month, k);
			SchDay sd = schedService.generateSchDay(sessSuid, k, sdate, i % 7, 0);
			week.add(sd);
			if (i % 7 == 6)
				calendar.add(week);
		}
		...
		if (lastDate != 6) {
			...
			for (int i = lastDate + 1, k = 1; i < 7; i++, k++) {
				...
				SchDay sd = schedService.generateSchDay(sessSuid, k, sdate, i, 1);
				week.add(sd);
			}
			calendar.add(week);
		}
